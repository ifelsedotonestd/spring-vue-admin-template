...
import one.ifelse.module.base.domain.token.exception.InvalidAudienceTokenException
import one.ifelse.module.base.domain.token.exception.RevokedJwtTokenException
import one.ifelse.module.base.domain.token.exception.UnknownIssuerTokenException
import one.ifelse.module.base.domain.user.exception.UserNotExistedException
...
class DomainJwtTokenProviderService(
    ...
    private fun buildPrincipalForRefreshTokenFromUser(userRef: Any): UserDetails {
        val userDetails: SecuredUserDetails = when (userRef) {
            ...
            String -> userDetailsService.loadUserByPreferredUsername(userRef as String)
            else -> throw UserNotExistedException()
        }
    ...
    companion object {
        ...
        private fun getClaimsWithoutKey(jwt: String): Map<String, String> {
            return try {
                val decoder = Base64.getUrlDecoder()
                val payload =
                    decoder.decode(jwt.split("\\.".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[1])
                JACKSON_MAPPER.readValue(payload, object : TypeReference<Map<String, String>>() {})
            } catch (e: Exception) {
                log.error("Cannot parse payload in token", e)
                throw JwtTokenException()
            }
